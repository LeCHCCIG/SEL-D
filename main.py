...
def train(args, data_generator, model, optimizer, logging):
    ...
    temp_submissions_dir_train = os.path.join(
        temp_submissions_dir, 'trial_{}'.format(trial), 'train')
    temp_submissions_dir_valid = os.path.join(
        temp_submissions_dir, 'trial_{}'.format(trial), 'valid')
    ...
    for batch_idx, (batch_x, batch_y_dict) in iterator:
        ...
        if batch_idx % 200 == 0:
            ...
            if args.fold != -1:
                ...
                logging_and_writer(
                    'valid', metrics, logging, writer, batch_idx)
            else:
                logging_and_writer('train', train_metrics,
                                   logging, writer, batch_idx)
        ...
        # Save model
        ###############
        # if batch_idx % save_interval == 0 and batch_idx > 4000:
        # Define the checkpoint iterations
        ...
        # Loss
        seld_loss, _, _ = hybrid_regr_loss(
            output, batch_y_dict, args.task_type, loss_type=loss_type)
...
def inference(args, data_generator, logging):
    ...
    if args.task_type == 'two_staged_eval':
        ...
        model_path = os.path.join(models_dir, 'sed_only',
                                  'model_' + Model_SED +
                                  '_{}'.format(
                                      args.audio_type) + '_fold_{}'.format(args.fold) + '_seed_{}'.format(args.seed),
        ...
        print(f"model testing path : {model_path}")
        ...
        os.makedirs(fold_submissions_dir, exist_ok=False)
        print(f"model testing path : {model_path}")
        print(f"Submission path: {fold_submissions_dir}")
        test_metrics = evaluation.evaluate(
            data_generator=data_generator,
            data_type='test',
            max_audio_num=None,
...
def inference_all_fold(args):
    ...
    gt_meta_dir = '/home/kelelu/Third_Paper/third/seld-dcase2019/dataset/metadata_dev/'
    sed_scores, doa_er_metric, seld_metric = evaluation.calculate_SELD_metrics(
        gt_meta_dir, test_submissions_dir, score_type='all')
...
if __name__ == '__main__':
    ...
    parser_inference.add_argument('--fold', default=1, type=int,
                                  help='fold for cross validation, if -1, use full data')
    parser_inference.add_argument('--iteration', default=4000, type=int,
    ...
    parser_testaudio = subparsers.add_parser('testaudio')
    parser_testaudio.add_argument('--workspace', type=str, required=True,
                                  help='workspace directory')
    parser_testaudio.add_argument('--feature_dir', type=str, required=True,
                                  help='feature directory')
    parser_testaudio.add_argument('--feature_type', type=str, required=True,
                                  choices=['logmel', 'logmelgcc'])
    parser_testaudio.add_argument('--audio_type', type=str, required=True,
                                  choices=['foa', 'mic'], help='audio type')
    parser_testaudio.add_argument('--task_type', type=str, required=True,
                                  choices=['sed_only', 'doa_only', 'two_staged_eval', 'seld'])
    ...
    if args.cuda:
        ...
        logging.info('\nNumber of GPU available: {}'.format(
