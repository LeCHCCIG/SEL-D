...
def evaluate(data_generator, data_type, max_audio_num, task_type, model, cuda, loss_type,
    ...
    for batch_x, batch_y_dict, batch_fn in generate_func:
        ...
        if task_type == 'sed_only':
            with h5py.File(temp_hdf5_path, 'w') as hf:
                hf.create_dataset(
                    'sed_pred', data=output['events'], dtype=np.float32)
        ...
        min_idx = min(batch_y_dict['events'].shape[0],
                      output['events'].shape[1])

        sed_gt.append(batch_y_dict['events'][:min_idx])
        doa_gt.append(batch_y_dict['doas'][:min_idx])

        sed_pred.append(
            (output['events'] > threshold['sed']).squeeze().astype(np.float32)[:min_idx])
    ...
    sed_scores, doa_er_metric, seld_metric = calculate_SELD_metrics(
        gt_meta_dir, submissions_dir, score_type='all')
    ###################################################################################################
    ...
    seld_loss, sed_loss, doa_loss = hybrid_regr_loss(
        forLoss_pred_dict, forLoss_gt_dict, task_type, loss_type=loss_type)
...
def calculate_submission(output_dict, frames_per_1s, sub_frames_per_1s=50):
    ...
    output_dict['events'] = interp_tensor(
        output_dict['events'], frames_per_1s, sub_frames_per_1s)
    output_dict['doas'] = interp_tensor(
        output_dict['doas'], frames_per_1s, sub_frames_per_1s)
...
def calculate_SELD_metrics(gt_meta_dir, pred_meta_dir, score_type):
    ...
    pred_meta_files = [fn for fn in os.listdir(
        pred_meta_dir) if fn.endswith('.csv') and not fn.startswith('.')]
